...
@implementation EUJailbreaker

- (NSError *)gatherSystemInformation
{
    NSString *kernelPath = [[EUEnvironmentManager sharedManager] accessibleKernelPath];
    if (!kernelPath) return [NSError errorWithDomain:JBErrorDomain code:JBErrorCodeFailedToFindKernel userInfo:@{NSLocalizedDescriptionKey:@"Failed to find kernelcache. Ensure your device is properly connected to the internet. If it still does not work, try installing Euphoria via TrollStore instead."}];
    NSLog(@"Kernel at %@", kernelPath);

    NSString *sptmPath = [[EUEnvironmentManager sharedManager] accessibleSPTMPath];
    if (sptmPath) {
        NSLog(@"SPTM at %@", sptmPath);
    }
    NSString *txmPath = [[EUEnvironmentManager sharedManager] accessibleTXMPath];
    if (txmPath) {
        NSLog(@"TXM at %@", txmPath);
    }
    
    [[EUUIManager sharedInstance] sendLog:EULocalizedString(@"Patchfinding") debug:NO];
    ...
    // This will later allow launchdhook to figure out which process is the euphoria app
...
{
    EUExploit *kernelExploit = [EUExploitManager sharedManager].selectedKernelExploit;
    EUExploit *pacBypass     = [EUExploitManager sharedManager].selectedPACBypass;
    EUExploit *pplBypass     = [EUExploitManager sharedManager].selectedPPLBypass;
    ...
    if (!pacBypass && [EUEnvironmentManager sharedManager].isPACBypassRequired) {
    ...
    if (!pplBypass && [EUEnvironmentManager sharedManager].isPPLBypassRequired) {
    ...
    [[EUUIManager sharedInstance] sendLog:[NSString stringWithFormat:EULocalizedString(@"Exploiting Kernel (%@)"), kernelExploit.name] debug:NO];
    ...
    if (pacBypass) {
        [[EUUIManager sharedInstance] sendLog:[NSString stringWithFormat:EULocalizedString(@"Bypassing PAC (%@)"), pacBypass.name] debug:NO];
    ...
    if ([[EUEnvironmentManager sharedManager] isPPLBypassRequired]) {
        if ([EUEnvironmentManager sharedManager].isSPTM) {
            [[EUUIManager sharedInstance] sendLog:[NSString stringWithFormat:EULocalizedString(@"Bypassing SPTM (%@)"), pplBypass.name] debug:NO];
        }
        else {
            [[EUUIManager sharedInstance] sendLog:[NSString stringWithFormat:EULocalizedString(@"Bypassing PPL (%@)"), pplBypass.name] debug:NO];
        }
    ...
    if (![EUEnvironmentManager sharedManager].isArm64e) {
        arm64_kcall_init();
    }
...
{
    int r = [[EUExploitManager sharedManager] cleanUpExploits];
...
{
    int r = [[EUEnvironmentManager sharedManager] setPrivatePrebootProtected:YES];
    if (r != 0) {
...
{
    NSMutableSet *euphoriaInstalledAppIds = [NSMutableSet new];
    NSMutableSet *userInstalledAppIds = [NSMutableSet new];
    
    NSString *euphoriaAppsPath = JBROOT_PATH(@"/Applications");
    NSString *userAppsPath = @"/var/containers/Bundle/Application";
    
    for (NSString *euphoriaAppName in [[NSFileManager defaultManager] contentsOfDirectoryAtPath:euphoriaAppsPath error:nil]) {
        NSString *infoPlistPath = [[euphoriaAppsPath stringByAppendingPathComponent:euphoriaAppName] stringByAppendingPathComponent:@"Info.plist"];
        NSDictionary *infoDictionary = [NSDictionary dictionaryWithContentsOfFile:infoPlistPath];
        NSString *appId = infoDictionary[@"CFBundleIdentifier"];
        if (appId) {
            if (![euphoriaInstalledAppIds containsObject:appId]) {
                [euphoriaInstalledAppIds addObject:appId];
            }
            else {
                return [NSError errorWithDomain:JBErrorDomain code:JBErrorCodeFailedDuplicateApps userInfo:@{ NSLocalizedDescriptionKey : [NSString stringWithFormat:EULocalizedString(@"Duplicate_Apps_Error_Euphoria_App"), appId, euphoriaAppsPath]}];
            }
        }
    }
    ...
    NSMutableSet *duplicateApps = euphoriaInstalledAppIds.mutableCopy;
    ...
    if (duplicateApps.count) {
        ...
        [duplicateAppsString appendString:@"]"];
        return [NSError errorWithDomain:JBErrorDomain code:JBErrorCodeFailedDuplicateApps userInfo:@{ NSLocalizedDescriptionKey : [NSString stringWithFormat:EULocalizedString(@"Duplicate_Apps_Error_User_App"), duplicateAppsString, euphoriaAppsPath]}];
    }
    
    for (NSString *euphoriaAppId in euphoriaInstalledAppIds) {
        LSApplicationProxy *appProxy = [LSApplicationProxy applicationProxyForIdentifier:euphoriaAppId];
        if (appProxy.installed) {
            ...
            if (![appProxyPath hasPrefix:euphoriaAppsPath]) {
                return [NSError errorWithDomain:JBErrorDomain code:JBErrorCodeFailedDuplicateApps userInfo:@{ NSLocalizedDescriptionKey : [NSString stringWithFormat:EULocalizedString(@"Duplicate_Apps_Error_Icon_Cache"), euphoriaAppId, euphoriaAppsPath, appProxy.bundleURL.path]}];
            }
        }
    }
...
- (NSError *)removeJailbreak
{
    if (@available(iOS 18.0, *)) {
        ...
        [[EUUIManager sharedInstance] sendLog:EULocalizedString(@"Rebuilding Icon Cache") debug:NO];
        [[EUEnvironmentManager sharedManager] rebuildIconCache];
    }

    NSError *err;
    [[EUUIManager sharedInstance] sendLog:EULocalizedString(@"Removing Jailbreak") debug:NO];
    err = [[EUEnvironmentManager sharedManager] deleteBootstrap];
    if (err) return err;
...
{
    return [[EUEnvironmentManager sharedManager] finalizeBootstrap];
}
...
- (void)runWithError:(NSError **)errOut didRemoveJailbreak:(BOOL*)didRemove showLogs:(BOOL *)showLogs
{
    ...
    BOOL tweaksEnabled = [[EUPreferenceManager sharedManager] boolPreferenceValueForKey:@"tweakInjectionEnabled" fallback:YES];
    BOOL idownloadEnabled = [[EUPreferenceManager sharedManager] boolPreferenceValueForKey:@"idownloadEnabled" fallback:NO];
    BOOL appJITEnabled = [[EUPreferenceManager sharedManager] boolPreferenceValueForKey:@"appJITEnabled" fallback:YES];
    NSNumber *jetsamMultiplierOption = [[EUPreferenceManager sharedManager] preferenceValueForKey:@"jetsamMultiplier"];
    ...
    [[EUUIManager sharedInstance] sendLog:startLog debug:YES];
    ...
    *errOut = [self doExploitation];
    ...
    [[EUUIManager sharedInstance] sendLog:EULocalizedString(@"Building Phys R/W Primitive") debug:NO];
    *errOut = [self buildPhysRWPrimitive];
    if (*errOut) {
        [self cleanUpExploits];
        return;
    }
    [[EUUIManager sharedInstance] sendLog:EULocalizedString(@"Cleaning Up Exploits") debug:NO];
    *errOut = [self cleanUpExploits];
    if (*errOut) return;
    
    // We will not be able to reset this after elevating privileges, so do it now
    if (removeJailbreakEnabled) [[EUPreferenceManager sharedManager] setPreferenceValue:@NO forKey:@"removeJailbreakEnabled"];

    [[EUUIManager sharedInstance] sendLog:EULocalizedString(@"Elevating Privileges") debug:NO];
    ...
    *errOut = [[EUEnvironmentManager sharedManager] ensureJailbreakRootExists];
    if (*errOut) {
        [self cleanUpPostExploitation];
        return;
    }
    
    *errOut = [[EUEnvironmentManager sharedManager] prepareBootstrap];
    if (*errOut) return;
    setenv("PATH", "/sbin:/bin:/usr/sbin:/usr/bin:/var/jb/sbin:/var/jb/bin:/var/jb/usr/sbin:/var/jb/usr/bin", 1);
    setenv("TERM", "xterm-256color", 1);

    *errOut = [[EUEnvironmentManager sharedManager] updateBootLogo];
    if (*errOut) {
        [self cleanUpPostExploitation];
        return;
    }
    ...
    [[EUUIManager sharedInstance] sendLog:EULocalizedString(@"Loading BaseBin TrustCache") debug:NO];
    *errOut = [self loadBasebinTrustcache];
    ...
    [[EUUIManager sharedInstance] sendLog:EULocalizedString(@"Initializing Environment") debug:NO];
    *errOut = [self injectLaunchdHook];
    ...
    [[EUEnvironmentManager sharedManager] setJailbroken:YES withVersion:[NSString stringWithContentsOfFile:JBROOT_PATH(@"/basebin/.version") encoding:NSUTF8StringEncoding error:nil]];
    ...
    [[EUUIManager sharedInstance] sendLog:EULocalizedString(@"Initializing Protection") debug:NO];
    *errOut = [self applyProtection];
    ...
    [[EUUIManager sharedInstance] sendLog:EULocalizedString(@"Applying Bind Mount") debug:NO];
    ...
    *errOut = [self finalizeBootstrapIfNeeded];
    ...
    [[EUEnvironmentManager sharedManager] setIDownloadEnabled:idownloadEnabled needsUnsandbox:NO];
    
    [[EUUIManager sharedInstance] sendLog:EULocalizedString(@"Checking For Duplicate Apps") debug:NO];
...
- (void)finalize
{
    [[EUUIManager sharedInstance] sendLog:EULocalizedString(@"Rebooting Userspace") debug:NO];
...
- (void)applyContiguousMappingWorkaround
{
    ...
    // Once it is freed, the next Euphoria can regain the contiguous mapping
