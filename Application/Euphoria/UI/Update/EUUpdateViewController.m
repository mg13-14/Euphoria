...
@property (strong, nonatomic) EUActionMenuButton *button;
...
@implementation EUUpdateViewController
...
- (void)viewDidLoad {
    ...
    title.text = EULocalizedString(@"Title_Changelog");
    ...
    BOOL envUpdate = [[EUUIManager sharedInstance] environmentUpdateAvailable];
    
    self.button = [EUActionMenuButton buttonWithAction:[UIAction actionWithTitle:EULocalizedString(envUpdate ? @"Button_Update_Environment" : @"Button_Update") image:[UIImage systemImageNamed:@"arrow.down" withConfiguration:[EUGlobalAppearance smallIconImageConfiguration]] identifier:@"update" handler:^(__kindof UIAction * _Nonnull action) {
        if (envUpdate)
        {
            self.button.enabled = NO;
            self.button.alpha = 0.5;
            NSError *error = [[EUEnvironmentManager sharedManager] updateEnvironment];
            if (error)
            {
                ...
                [alert addAction:[UIAlertAction actionWithTitle:EULocalizedString(@"Button_Close") style:UIAlertActionStyleDefault handler:nil]];
                [self presentViewController:alert animated:YES completion:nil];
            }
            return;
        }

        if (![EUEnvironmentManager sharedManager].isJailbroken || [[EUUIManager sharedInstance] launchedReleaseNeedsManualUpdate] || ![EUEnvironmentManager sharedManager].isInstalledThroughTrollStore)
        {
            if ([EUEnvironmentManager sharedManager].isInstalledThroughTrollStore) {
        ...
        EUDownloadViewController *downloadVC = [[EUDownloadViewController alloc] initWithUrl:self.lastestDownloadUrl callback:^(NSURL * _Nonnull file) {
            NSLog(@"Downloaded %@", file);
            [[EUEnvironmentManager sharedManager] updateJailbreakFromTIPA:file.path];
        }];
...
{
    NSArray *releases = [[EUUIManager sharedInstance] getUpdatesInRange:self.fromTag end:self.toTag];
    NSMutableParagraphStyle * paragraphStyle = [[NSMutableParagraphStyle alloc] init];
    paragraphStyle.alignment = NSTextAlignmentCenter;
    ...
    if (releases.count == 0)
    {
        [changelogText appendAttributedString:[[NSAttributedString alloc] initWithString:EULocalizedString(@"Changelog_Unavailable_Text") attributes:@{NSFontAttributeName: [UIFont systemFontOfSize:18], NSForegroundColorAttributeName : [UIColor whiteColor], NSParagraphStyleAttributeName:paragraphStyle}]];
