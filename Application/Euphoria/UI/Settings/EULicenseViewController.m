...
@interface EULicenseViewController ()
...
@implementation EULicenseViewController
...
- (void)viewDidLoad
{
    ...
    UIView *header = [EUPSListItemsController makeHeader:EULocalizedString(@"Credits_Button_License") withTarget:self];
...
{
    NSArray *licenses = [EULicenseViewController licenses];
...
- (void)setSelectedLicense:(int)selectedLicense
{
    ...
    self.license.text = [NSString stringWithContentsOfFile:[[NSBundle mainBundle] pathForResource:[EULicenseViewController licenses][_selectedLicense][@"file"] ofType:@"md"] encoding:NSUTF8StringEncoding error:nil];
