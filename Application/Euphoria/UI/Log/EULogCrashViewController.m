...
@interface EULogCrashViewController ()

@property (nonatomic, retain) NSString *title;

@end

@implementation EULogCrashViewController
...
{
    [super viewDidLoad];
    [EUPSListController setupViewControllerStyle:self];

    UIView *header = [EUPSListItemsController makeHeader:EULocalizedString(@"Log_Error") withTarget:self];
    ...
    __block EUActionMenuButton *shareButton;
    UIAction *shareAction = [UIAction actionWithTitle:EULocalizedString(@"Button_Share") image:[UIImage systemImageNamed:@"square.and.arrow.up" withConfiguration:[EUGlobalAppearance smallIconImageConfiguration]] identifier:@"share" handler:^(__kindof UIAction * _Nonnull action) {
        [[EUUIManager sharedInstance] shareLogRecordFromView:shareButton];
    }];
    shareButton = [EUActionMenuButton buttonWithAction:shareAction chevron:NO];
    ...
    NSArray *reverseLog = [[[EUUIManager sharedInstance] logRecord] reverseObjectEnumerator].allObjects;
