...
- 状态：已采纳（清单口径已由用户裁定：与 Dopamine 安装默认一致，18:32:59）；
  **2026-09-05 19:44:48 汇总员裁定 a+b+c 组合修订**（用户实报"自带源可被删"后的
  结构性勘误，见 §6）
- 需求：R14（01-需求追溯 第48行）："刚开始要记得加好固定的几个源，不能删"
- 决策人：B（机制设计）／实施：C（预置写入）＋ B（bootstrapfs 侧重放点）
- 日期：2026-08-26（v2 修订：2026-09-05，C）
...
## 6. v2 修订（2026-09-05：用户实报失效后的结构性勘误）

**触发**：用户实报"自带越狱源可被删"（Sileo 源页删除成功）。C 定位三层失效（19:43:35），汇总员 19:44:48 裁定 a+b+c 组合处置。

### 6.1 根因（层1 假设被证伪的部分）

§1.1 的"UI 删除源=unlink .list 文件"在 **Sileo 2.x 上不成立**：其源管理走
**自建 db 层**（Sileo 私有数据库的源条目表），用户在源页"删除"=db 条目移除，
**不产生 unlink 系统调用**——chflags 文件锁（层2）在 Sileo 2.x 上结构性失效
（锁了个寂寞）。该假设对 Zebra（真走文件）仍成立。

### 6.2 三层失效窗口（C 00:43 定位全录）

1. **Sileo 2.x db 层**（根因，如上）；
2. **procursus.sources 裸奔窗口**：Sileo 首启自播种该文件，锁定动作滞后到
   下次越狱重放（首启时机不可知，无法提前锁定）；
3. **presetSources 进程缓存**（低危）：dispatch_once 缓存模式切换不刷新，
   重启进程即消失。

### 6.3 v2 决策（a+b+c 组合，汇总员裁定）

- **a. 如实修订（本节）**：文件层防呆保留（chflags 照旧——拦 Zebra 文件路径
  删除与手滑 rm）；**不 hack Sileo 自建 db**（不逆向/不魔改第三方包管理器的
  私有存储——正派边界：我们的软件不碰别人软件的私有数据库）。
- **b. 结构性正解=源显示层绑 EPM**：**文件层 = SSOT（唯一事实源）**——
  EPM 仙市页设"固定源区"（不可删，C 接文件层后端）+ 每次启动直读
  sources.list.d 文件层渲染；越狱幂等重放（层2 既有机制）= 文件层自愈闭环。
  效果：Sileo 里删条目只删它自己的 db 视图，真源永远在文件层，下次越狱/
  EPM 打开即恢复显示。**用户"删不掉"的完整体验由 EPM 层兑现**（B 落 UI）。
- **c. 裸奔窗口如实记录**：procursus.sources 首启播种到下次越狱重放之间
  无锁（接受；用户主动 SSH 删属于"防呆不防盗"既有边界）。

### 6.4 分工（v2）

| 项 | 负责 |
|---|---|
| ADR v2 修订（本节） | C ✅ 2026-09-05 |
| EPM 固定源区文件层后端（直读 sources.list.d + SSOT 渲染） | C |
| EPM 仙市页固定源区 UI（不可删交互） | B |
| 层2 chflags 重放（既有，不动） | C/B（原分工不变） |
