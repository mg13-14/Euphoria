...
bool jbclient_euphoria_is_jailbroken(char **version)
{
	xpc_object_t xreply = jbserver_xpc_send(JBS_DOMAIN_EUPHORIA, JBS_EUPHORIA_IS_JAILBROKEN, NULL);
	if (xreply) {
		int64_t result = xpc_dictionary_get_int64(xreply, "result");
...
int jbclient_euphoria_get_root(void)
{
	xpc_object_t xreply = jbserver_xpc_send(JBS_DOMAIN_EUPHORIA, JBS_EUPHORIA_GET_ROOT, NULL);
	if (xreply) {
		int64_t result = xpc_dictionary_get_int64(xreply, "result");
		xpc_release(xreply);
		return (bool)result;
...
int jbclient_euphoria_drop_root(void)
{
	xpc_object_t xreply = jbserver_xpc_send(JBS_DOMAIN_EUPHORIA, JBS_EUPHORIA_DROP_ROOT, NULL);
	if (xreply) {
		int64_t result = xpc_dictionary_get_int64(xreply, "result");
		xpc_release(xreply);
		return (bool)result;
