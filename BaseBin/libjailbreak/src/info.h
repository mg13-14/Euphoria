...
struct system_info {
	...
	struct {
		...
		// Euphoria rootful user toggle (App Settings UI, 2026-08-26 17:27):
		//   false (default) -> roothide mode: rootless + cloak/aegis hiding
		//   true            -> rootful + roothide (rootful engine + hiding)
	...
	struct {
		...
		uint64_t mount_list; // TAILQ_HEAD(_mount_list) — rootful engine
	...
	struct {
		...
		struct {
			// Rootful engine: XNU struct mount (validated at runtime
			// against f_mntonname == "/" before any write happens)
			uint32_t mnt_next;      // TAILQ_ENTRY(mount) mnt_list.tqe_next
			uint32_t mnt_flag;      // uint32_t mnt_flag
			uint32_t mnt_kern_flag; // uint32_t mnt_kern_flag (MNTK_*)
			uint32_t mnt_vfsstat;   // struct vfsstatfs mnt_vfsstat
			uint32_t mnt_data;      // void *mnt_data
		} mount;

		struct {
			// XNU struct vfsstatfs (offsets are relative to the struct
			// itself, add koffsetof(mount, mnt_vfsstat))
			uint32_t f_mntonname;   // char f_mntonname[MAXPATHLEN]
...
#define JAILBREAK_SETTINGS_ITERATE(ctx, iterator) \
	...
	iterator(ctx, jailbreakSettings.jetsamMultiplier); \
        iterator(ctx, jailbreakSettings.cloakEnabled); \
        iterator(ctx, jailbreakSettings.cloakHideMounts); \
...
#define KERNEL_SYMBOLS_ITERATE(ctx, iterator) \
	...
	iterator(ctx, kernelSymbol.mount_list); \
