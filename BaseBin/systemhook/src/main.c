...
bool should_enable_tweaks(void)
{
	...
	if (jbclient_euphoria_is_jailbroken(NULL)) {
		// Probe whether we are the Euphoria app
		// Only the Euphoria app is allowed to contact this domain
...
int parse_dyldhook_jbinfo(char **jbRootPathOut, char **bootUUIDOut, char **sandboxExtensionsOut, bool *fullyDebuggedOut)
{
	...
	// Check if dyld LC_UUID contains euphoria magic
	uuid_t dyldUUID;
	if (!_dyld_get_image_uuid((const struct mach_header *)dyldHeader, dyldUUID)) return -2;
	if (!string_has_prefix((char *)dyldUUID, "EUPH")) return -3;
...
#endif
		// Euphoria cloak: hide jailbreak mounts and elevated credentials
		// from processes that should not be able to see them
		cloak_interpose_init();
		aegis_interpose_init();
