...
__attribute__((constructor)) static void initializer(void)
{
	...
	if (getenv("EUPHORIA_INITIALIZED") != 0) {
		// If Euphoria was initialized before, we assume we're coming from a userspace reboot
	...
	if (err != 0) {
		char msg[1000];
		snprintf(msg, 1000, "Euphoria: Failed to recover primitives (error %d), cannot continue.", err);
	...
	if (getenv("EUPHORIA_IS_HIDDEN") != 0) {
	...
	// Mark Euphoria as having been initialized before
	setenv("EUPHORIA_INITIALIZED", "1", 1);
