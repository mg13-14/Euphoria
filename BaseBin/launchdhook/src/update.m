...
void jbupdate_update_system_info(void)
{
	@autoreleasepool {
		...
		if (!xpfHandle) {
			char msg[4000];
			snprintf(msg, 4000, "Euphoria: dlopening libxpf failed: (%s), cannot continue.", dlerror());
		...
		if (error) {
			char msg[4000];
			snprintf(msg, 4000, "Euphoria: Updating system info via XPF failed with error: (%s), cannot continue.", error);
			abort_with_reason(7, 1, msg, 0);
			return;
		}
...
void jbupdate_finalize_stage2(const char *prevVersion, const char *newVersion)
{
	...
	if (strcmp(prevVersion, "2.4") < 0 && strcmp(newVersion, "2.4") >= 0) {
		// On Euphoria <= 2.3, dyld used to be a file on the fakelib mount
	...
	if (strcmp(prevVersion, "3.0") < 0 && strcmp(newVersion, "3.0") >= 0) {
		// Euphoria 3.0 changed PPLRW_USER_MAPPING_OFFSET and a jbupdate from 2.x to >=3.x is therefore unsupported
		reboot(0);
	}
	...
	if (r != 0) {
		char msg[4000];
		snprintf(msg, 4000, "Euphoria: Updating patched dyld failed with error %d, cannot continue.", r);
		abort_with_reason(7, 1, msg, 0);
	}
	...
	if (cdhashesCount > 1) {
		char msg[4000];
		snprintf(msg, 4000, "Euphoria: Updating patched dyld failed due to unexpected amount of cdhashes (%d), cannot continue.", cdhashesCount);
		abort_with_reason(7, 1, msg, 0);
	}
		...
		if (r != 0) {
			char msg[4000];
			snprintf(msg, 4000, "Euphoria: Building dyld trustcache failed with error %d, cannot continue.", r);
			abort_with_reason(7, 1, msg, 0);
		}
		...
		if (r != 0) {
			char msg[4000];
			snprintf(msg, 4000, "Euphoria: Updating dyld trustcache failed with error %d, cannot continue.", r);
			abort_with_reason(7, 1, msg, 0);
		}
