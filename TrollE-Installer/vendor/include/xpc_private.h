...
// 修复#7：jbserver.c 用的 mach_send 系（libxpc 私有 API，新版 SDK 头不再暴露）
extern void xpc_dictionary_set_mach_send(xpc_object_t dictionary, const char* name, mach_port_t port);
extern XPC_RETURNS_RETAINED mach_port_t xpc_dictionary_copy_mach_send(xpc_object_t dictionary, const char* name);
