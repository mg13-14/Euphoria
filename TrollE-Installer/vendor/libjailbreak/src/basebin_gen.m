...
int apply_dyld_patch(NSString *dyldPath, const char *newUUIDPrefix)
{
	...
	macho_enumerate_load_commands(dyldMacho, ^(struct load_command loadCommand, uint64_t offset, void *cmd, bool *stop) {
		if (loadCommand.cmd == LC_UUID) {
            ...
            // EUPH<euphoria version>\0<rest of original UUID>
            // This way we ensure:
            // - The version it was patched on and it being patched by Euphoria is identifiable later
...
int basebin_generate_internal(NSString *originUsrLibPath, NSString *basebinPath, NSString *targetBasebinPath, bool comingFromJBUpdate)
{
	...
	NSString *euphoriaVersion = [NSString stringWithContentsOfFile:versionPath encoding:NSUTF8StringEncoding error:nil];
	if (!euphoriaVersion) return 1;
	...
	NSString *dyldUUIDPrefix = [@"EUPH" stringByAppendingString:euphoriaVersion];
