# Installing Euphoria on a Corellium
...
2. Compile Euphoria
3. Run `codesign -dvvvv on BaseBin/euphoria/euphoria`
4. Copy the CandidateCDHash and paste it into Settings -> Trust Cache on the Non-Jailbroken Corellium instance
5. Run `env NODE_TLS_REJECT_UNAUTHORIZED=0 node euphoria.js install <Project> <Instance>` where `Project` and `Instance` can either be a name or an ID
